...
namespace KoTA206\Generator\Common;
...
class GeneratorConfig
{
    ...
    public function loadNamespaces(CommandData &$commandData)
    {
        ...
        $this->nsRepository = config('KoTA206.laravel_generator.namespace.repository', 'App\Repositories').$prefix;
        $this->nsModel = config('KoTA206.laravel_generator.namespace.model', 'App\Models').$prefix;
        if (config('KoTA206.laravel_generator.ignore_model_prefix', false)) {
            $this->nsModel = config('KoTA206.laravel_generator.namespace.model', 'App\Models');
        }
        $this->nsDataTables = config('KoTA206.laravel_generator.namespace.datatables', 'App\DataTables').$prefix;
        $this->nsModelExtend = config(
            'KoTA206.laravel_generator.model_extend_class',
            'Illuminate\Database\Eloquent\Model'
        );

        $this->nsApiController = config(
            'KoTA206.laravel_generator.namespace.api_controller',
            'App\Http\Controllers\API'
        ).$prefix;
        $this->nsApiRequest = config('KoTA206.laravel_generator.namespace.api_request', 'App\Http\Requests\API').$prefix;

        $this->nsRequest = config('KoTA206.laravel_generator.namespace.request', 'App\Http\Requests').$prefix;
        $this->nsRequestBase = config('KoTA206.laravel_generator.namespace.request', 'App\Http\Requests');
        $this->nsBaseController = config('KoTA206.laravel_generator.namespace.controller', 'App\Http\Controllers');
        $this->nsController = config('KoTA206.laravel_generator.namespace.controller', 'App\Http\Controllers').$prefix;
    }
    ...
    public function loadPaths()
    {
        ...
        $this->pathRepository = config(
            'KoTA206.laravel_generator.path.repository',
            app_path('Repositories/')
        ).$prefix;

        $this->pathModel = config('KoTA206.laravel_generator.path.model', app_path('Models/')).$prefix;
        if (config('KoTA206.laravel_generator.ignore_model_prefix', false)) {
            $this->pathModel = config('KoTA206.laravel_generator.path.model', app_path('Models/'));
        }

        $this->pathDataTables = config('KoTA206.laravel_generator.path.datatables', app_path('DataTables/')).$prefix;

        $this->pathApiController = config(
            'KoTA206.laravel_generator.path.api_controller',
            app_path('Http/Controllers/API/')
        ).$prefix;

        $this->pathApiRequest = config(
            'KoTA206.laravel_generator.path.api_request',
            app_path('Http/Requests/API/')
        ).$prefix;

        $this->pathApiRoutes = config('KoTA206.laravel_generator.path.api_routes', app_path('Http/api_routes.php'));

        $this->pathApiTests = config('KoTA206.laravel_generator.path.api_test', base_path('tests/'));

        $this->pathApiTestTraits = config('KoTA206.laravel_generator.path.test_trait', base_path('tests/traits/'));

        $this->pathController = config(
            'KoTA206.laravel_generator.path.controller',
            app_path('Http/Controllers/')
        ).$prefix;

        $this->pathRequest = config('KoTA206.laravel_generator.path.request', app_path('Http/Requests/')).$prefix;

        $this->pathRoutes = config('KoTA206.laravel_generator.path.routes', app_path('Http/routes.php'));

        $this->pathViews = config(
            'KoTA206.laravel_generator.path.views',
            base_path('resources/views/')
        ).$viewPrefix.$this->mSnakePlural.'/';

        $this->modelJsPath = config(
                'KoTA206.laravel_generator.path.modelsJs',
    ...
    public function loadDynamicVariables(CommandData &$commandData)
    {
        ...
        $commandData->addDynamicVariable(
            '$API_PREFIX$',
            config('KoTA206.laravel_generator.api_prefix', 'api')
        );

        $commandData->addDynamicVariable(
            '$API_VERSION$',
            config('KoTA206.laravel_generator.api_version', 'v1')
        );
    ...
    public function prepareOptions(CommandData &$commandData)
    {
        ...
        $this->options['softDelete'] = config('KoTA206.laravel_generator.options.softDelete', false);
    ...
    {
        $this->prefixes['route'] = explode('/', config('KoTA206.laravel_generator.prefixes.route', ''));
        $this->prefixes['path'] = explode('/', config('KoTA206.laravel_generator.prefixes.path', ''));
        $this->prefixes['view'] = explode('.', config('KoTA206.laravel_generator.prefixes.view', ''));
        $this->prefixes['public'] = explode('/', config('KoTA206.laravel_generator.prefixes.public', ''));
    ...
    {
        $this->addOns['swagger'] = config('KoTA206.laravel_generator.add_on.swagger', false);
        $this->addOns['tests'] = config('KoTA206.laravel_generator.add_on.tests', false);
        $this->addOns['datatables'] = config('KoTA206.laravel_generator.add_on.datatables', false);
        $this->addOns['menu.enabled'] = config('KoTA206.laravel_generator.add_on.menu.enabled', false);
        $this->addOns['menu.menu_file'] = config('KoTA206.laravel_generator.add_on.menu.menu_file', 'layouts.menu');
    }
}
